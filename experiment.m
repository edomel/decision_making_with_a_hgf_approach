...
temp = input(prompt, 's');
fprintf('In this first part you will either see a dog\n');
pause(3);
imshow(dog)
pause(1);
close
...
pause(2);
fprintf('If the dog is likely to have an owner than the cat is less likely to have one as\n');
pause(2);
fprintf('p(owner|dog) = p(no owner|cat)\n');
pause(2);
...
prompt = 'Press enter when you are ready to go on\n';
temp = input(prompt, 's');
fprintf('In this last part you will either hear this sound\n');
pause(3);
sound(sound_1);
